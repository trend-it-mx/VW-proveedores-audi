...
function DataTable({ rows, setActualizar, setData }) {
  ...
  const columns = useMemo(() => {
    const editarPregunta = async (index, ingestaUsuario) => {
      const res = await toast.promise(
        axios.put(
          `${config.url.API_URL}/api/ingesta/usuario/${ingestaUsuario.id}`,
          {
            ingestaUsuario: ingestaUsuario.ingestaUsuario,
            estatus:
              ingestaUsuario.estatus === 'ACTIVO' ? 'INACTIVO' : 'ACTIVO',
          }
        ),
        {
