...
const DataTable = dynamic(() => import('../../components/DataTableCalculo'), {
  ssr: false,
});
const CalculoDeTipoDeCambio = ({ data, error }) => {
  const [ingestaUsuarios, setIngestaUsuarios] = useState(
    data.map((ingestaUsuario) => ({
      id: ingestaUsuario.ORDEN,
      purchase_order: ingestaUsuario.ORDEN,
      proveedor: ingestaUsuario.NOMBRE_PROVEEDOR,
      total: formatoMexico.format(ingestaUsuario.TOTAL),
      unidad: ingestaUsuario.UNIDAD,
      conversion: formatoMexico.format(ingestaUsuario.CONVERSION),
      tipo_conversion: ingestaUsuario.TIPO_CONVERSION,
      subRows: undefined,
    }))
  );
  useEffect(() => {
                ...
                <div className="flex flex-wrap -mx-3">
                  <div className="flex-none w-auto max-w-full px-3 my-auto">
                    <div className="h-full">
                      <h5 className="mb-1 dark:text-white">Cálculo de tipo de cambio</h5>
                    </div>
                  </div>                
                </div>
              </div>
...
export const getServerSideProps = async () => {
  ...
  try {
    const res = await fetch(
      `${config.url.API_URL}/api/transformacion/calculo/`
    );
    data = await res.json();
  } catch {
    error = true;
...
CalculoDeTipoDeCambio.auth = true;
CalculoDeTipoDeCambio.roles = ['Key user'];

export default CalculoDeTipoDeCambio;
