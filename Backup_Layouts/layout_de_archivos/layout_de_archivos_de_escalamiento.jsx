...
const LayoutDeEscalamiento = ({ data, error }) => {
  useEffect(() => {
    ...
    <Main meta={<Meta title="Componentes" description="Componentes" />}>
      <div className="flex w-full flex-col items-center gap-4">
        <h1 className="w-full border-b-2 font-bold">
          Layout de archivo de escalamiento
        </h1>
        <Formik
          ...
          onSubmit={async (values) => {
            await toast.promise(
              axios.post(
                `${config.url.API_URL}/api/administracion/parametros/lay_archivo/?tabla=ESCALAMIENTO_VW`,
                values
              ),
              {
...
LayoutDeEscalamiento.auth = true;
export default LayoutDeEscalamiento;
