...
const DataTable = dynamic(
  () => import('../../components/DataTableFacturacion'),
  {
    ssr: false,
  }
);
const Facturacion = ({ data, error, dato }) => {
          ...
          <div class="table-responsive">
                  ...
                  <div className="flex flex-wrap -mx-3">
                    <div className="flex-none w-auto max-w-full px-3 my-auto">
                      <div className="h-full">
                        <h5 className="mb-1 dark:text-white">Facturacion</h5>
                      </div>
                    </div>
              ...
              <div className="flex w-full flex-col items-center">
                ...
                <Registro
                  ...
                  data={data}
                  setData={setIngestaUsuarios}
                  rutaFull="api/ingesta/facturas"
                  rutaID="api/ingesta/factura"
                  carpetaBucket="Facturacion"
                  tabla="FACTURAS"
                  layout={dato}
                />
            </div>          
...
export const getServerSideProps = async () => {
  ...
  let error = false;
  try {
    const res = await fetch(`${config.url.API_URL}/api/ingesta/facturas/`);
    const res2 = await fetch(
      `${config.url.API_URL}/api/administracion/parametros/lay_archivo/?tabla=FACTURAS/`
    );
    data = await res.json();
...
Facturacion.roles = ['Key user'];
export default Facturacion;
// # sourceMappingURL=usuarios.jsx.map
